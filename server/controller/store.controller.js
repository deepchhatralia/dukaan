...
const addStore = async (ctx) => {
  ...
  let { store_link, name, contact, isOpen, city, state, country } =
    ctx.request.body;
  ...
  await updateStaff(
    ...
    { $set: { store_id: new ObjectId(resp.insertedId) } }
  );
...
const getMerchantStore = async (ctx) => {
  ...
  if (!resp) {
    ctx.body = {
      success: false,
      data: resp,
      msg: "No stores associated with the merchant",
    };
    return;
  }
...
const updateStore = async (ctx) => {
  ...
  const resp = await updateStoreById(
    { store_id, merchant_id: userId },
    { store_link, name, contact, isOpen, location: { city, state, country } }
  );
  ...
  if (!resp) {
    ctx.body = {
      success: true,
      data: resp,
      msg: "Store ID doesnt exist or Not authorized",
    };
    return;
  }
