...
const findInvitedStaff = (filter) =>
  getDb.db(dbName).collection(collectionName).findOne(filter);

const updateInvitedStaff = (filter, update_fields, options = {}) =>
  getDb
    .db(dbName)
    .collection(collectionName)
    .updateOne(filter, update_fields, options);

const deleteInvitedStaff = (token) =>
  getDb.db(dbName).collection(collectionName).deleteOne(token);
