...
const getAllCustomerAddress = (customer_id) =>
    getDb.db(dbName).collection(collectionName).find(
        { customer_id: new ObjectId(customer_id) }
    ).toArray();

const insertAddress = (addressObject) =>
    getDb.db(dbName).collection(collectionName).insertOne(
        { ...addressObject }
    );

const updateAddress = (filter, addressObject) =>
    getDb.db(dbName).collection(collectionName).findOneAndUpdate(
        filter,
        { $set: { ...addressObject } },
